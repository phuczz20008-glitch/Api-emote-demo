...
async def pick_10_emotes_safe():
    ...
    async with _emote_lock:
        ...
        if len(available) < 10:
            _last_used_emotes.clear()
            available = EMOTE_POOL.copy()
        emotes = random.sample(available, 10)
        _last_used_emotes.update(emotes)
        return emotes
...
def join_team():
    ...
    return jsonify({
        "status": "success",
        "message": "Đã gửi lệnh thực hiện emote!",
        "developer": "@Convitduc1",
        "time": current_time,
        "thanks": "Cảm ơn bạn đã sử dụng bot"
    })
...
def pip3_team():
    ...
    return jsonify({
        "status": "success",
        "message": "Đã gửi 3 emote hành động 7-8!",
        "developer": "@Convitduc1",
        "time": current_time,
        "thanks": "Cảm ơn bạn đã sử dụng bot"
    })
...
async def perform_emote_10(team_code, uids):
    if not online_writer:
        return
    ...
    emotes = await pick_10_emotes_safe()
    for emo in emotes:
        for uid in uids:
            pkt = await Emote_k(int(uid), emo, key, iv, region)
            await SEndPacKeT(None, online_writer, 'OnLine', pkt)
...
def pip10_team():
    ...
    asyncio.run_coroutine_threadsafe(
        perform_emote_10(team_code, uids), loop
    )

    return jsonify({
        "status": "success",
        "message": "Đã gửi 10 emote hành động 7-8!",
        "developer": "@Convitduc1",
        "time": current_time,
        "thanks": "Cảm ơn bạn đã sử dụng bot"
    })
...
async def MaiiiinE():
    ...
    Uid , Pw = '4368389012','ngocduc_demo_CXFYJ'
    ...
    if not MajoRLoGinResPonsE: print("Tài khoản mục tiêu => Bị cấm hoặc Chưa đăng ký  ! ") ; return None
    ...
    os.system('clear')
    print(render('DEMO', colors=['white', 'green'], align='center'))
    print('')
    #print(' - ReGioN => {region}'.format(region))
    print(f" - UID BOT : {TarGeT} | BOT TÊN : {acc_name}\n")
...
async def StarTinG():
    while True:
        ...
        except asyncio.TimeoutError: print("Token lỗi ! , khải động lại")
        except Exception as e: print(f"thành công - {e} => khởi động lại bot ...")
